...
void rb(int lc, bitset<9>& used, vector<char>& letters, unordered_map<char, int>& li_val, int sz, vector<unordered_map<char, int>>& operandset, vector<vector<char>>& operands, vector<int>& opsz, vector<int>& vals, unordered_set<char>& allocated_lets, bool& ex){
    if (sz == lc){
        ...
        // if (li_val['E'] == 2 && li_val['L'] == 0 && li_val['M'] == 3 && li_val['O'] == 4 && li_val['U'] == 9){
        //     for (int i = 0; i < 3; i++) cout << vals[i] << endl;
        //     cout << used << endl;
        //     cout << result << endl;
        // }
        
        // if (li_val['A'] == 9 && li_val['B'] == 8 && li_val['C'] == 7){
        //     for (int i = 0; i < 3; i++) cout << vals[i] << endl;
        //     cout << used << endl;
        //     cout << result << endl;
        // }
...
void solve(){
    ...
    vector<unordered_set<char>> fill(3);
    ...
    for (auto &letter : lexi){
        ...
        lessc++;
        for (int i = 0; i < 3; i++){
            if (operandset[i].find(letter) != operandset[i].end()){
                fill[i].insert(letter);
            }
        }
        vector<int> cnt = {0, 0, 0};
        int csm = 0;
        for (int i = 0; i < 3; i++){
            if (fill[i].size() == operandset[i].size())
                {cnt[i]++; csm++;}
        }
        // cout << "csm : " << csm << endl;
        if (csm == 2){
