...
void rb(bitset<9>& used, vector<char>& letters, unordered_map<char, int>& li_val, int sz, char& operation, vector<int>& opsz, vector<int>& vals, vector<unordered_map<char, int>>& operandset){
    if (sz == c){
...
void solve(){
    ...
    for (auto &ch : s){
        if (ops.find(ch) == ops.end()){
            if (operandset[si].find(ch) == operandset[si].end())
                operandset[si][ch] = pow(10, opsz[si] - pv_inv);
            else
                operandset[si][ch] += pow(10, opsz[si] - pv_inv);
            pv_inv ++;
        }
    ...
    rb(tempb, letters, li_val, 0, operation, opsz, vals, operandset);
