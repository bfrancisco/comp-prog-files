        ...
        for (int i = 0; i < 4; i++){
            ...
            if (i == 1){   
                if (valid(cur.fi, cur.se+1, grid, goal)){   // E
                    if (valid(cur.fi-1, cur.se+1, grid, goal))  // NE
                        open.insert({man+3, "ENW", newp});
                    // EW
                    open.insert({man+3, "EWN", newp});
                }
                else if (valid(cur.fi, cur.se-1, grid, goal)){  // W
                    open.insert({man+3, "WEN", newp});
                }
            }
            ...
            //S sidestep
                ...
                if (valid(cur.fi, cur.se+1, grid, goal)){ // E
                    if (valid(cur.fi+1, cur.se+1, grid, goal)) // SE
                        open.insert({man+3, "ESW", newp});
                    // EW
                    open.insert({man+3, "EWS", newp});
                }
                else if (valid(cur.fi, cur.se-1, grid, goal)){  // W
                    open.insert({man+3, "WES", newp});
                }
            }
            ...
            //W sidestep
                ...
                if (valid(cur.fi-1, cur.se, grid, goal)) // N
                    open.insert({man+3, "NSW", newp});
                else if (valid(cur.fi+1, cur.se, grid, goal)) // S
                    open.insert({man+3, "SNW", newp});
            }
            ...
            //E sidestep
                ...
                if (valid(cur.fi-1, cur.se, grid, goal)){ // N
                    if (valid(cur.fi-1, cur.se+1, grid, goal))  // NE
                        open.insert({man+3, "NES", newp});
                    // NS
                    open.insert({man+3, "NSE", newp});
                }
                else if (valid(cur.fi+1, cur.se, grid, goal)){ // S
                    if (valid(cur.fi+1, cur.se+1, grid, goal)){ // SE
                        open.insert({man+3, "SEN", newp});
                    }
                    // SN
                    open.insert({man+3, "SNE", newp});
                }
            }
                
            if (dtoi[prev_move.back()] != i)
                open.insert({man+1, dir[i], newp});
